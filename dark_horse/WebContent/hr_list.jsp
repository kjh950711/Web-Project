					 ...
					 <div class="row">
						...
						<tbody>
							<c:forEach items="${hlist}" var="hivo" varStatus="status">
									<tr>
										<td>${hivo.rownum}</td>
										<td><img src="${hivo.h_photo}" class="rounded"
											alt="Cinque Terre" width="80" height="50"></td>
										<td><a href="/dark_horse/horse_detail?hrname=${hivo.hrname}" >${hivo.hrname}</a></td>
										<td>${hivo.hrno}</td>
										<td>${hivo.h_sex}</td>
										<td>${hivo.h_birthday}</td>
										<td>${hivo.place}</td>
										<td>${hivo.h_win_rate}%</td>
										<td>${hivo.training_cnt}</td>
									</tr>
							</c:forEach>
						</tbody>
						</table>
