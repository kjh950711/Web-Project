...
<link rel="stylesheet"
	href="resources/assets/vendor/bootstrap-select/css/bootstrap-select.css">
...
<script src="https://kit.fontawesome.com/7e5d0aa0b2.js"
	crossorigin="anonymous"></script>

</head>

<body>
	<input type="hidden" name="gamePlace" id="place">
	<input type="hidden" name="gameDate" id="g_date">
	<input type="hidden" name="gameRound" id="g_round">
	<input type="hidden" name="select_opt" id="select_opt">

	<div class="dashboard-main-wrapper">
		<div class="dashboard-header">
			<div class="logo_header">
				<a href="/dark_horse/main"><p id="brand">
						DARK<br> HORSE
					</p></a>
			</div>
			<nav class="navbar navbar-expand-sm bg-dark navbar-dark sticky-top">
				<ul class="navbar-nav">
					<li class="nav-item"><a class="nav-link" href="main">금주의
							경마</a></li>
					<li class="nav-item"><a class="nav-link" href="horse_list">경주마</a>
					</li>
					<li class="nav-item"><a class="nav-link" href="player_list">기수</a>
					</li>
				</ul>
				<form class="form-inline my-2 my-lg-0" method="get" action=""
					...
					<label for="sel1"></label> <select class="form-control"
						...
						placeholder="Search" aria-label="Search" id="t_search"
						name="pName">
			...
			<div class="dashboard-ecommerce">
				<div class="container-fluid">
					...
					<div class="ecommerce-widget">
							...
							<div class="col-xl-7 col-lg-8 col-md-8 col-sm-8 col-8">
								...
								<div class="simple-card">
									<ul class="nav nav-tabs" id="myTab5" role="tablist">
										<li class="nav-item myclick" param1="서울"><a
											class="nav-link active border-left-0" id="home-tab-simple"
											data-toggle="tab" href="#home-simple" role="tab"
											aria-controls="home" aria-selected="true">서울</a></li>
										<li class="nav-item myclick" param1="부경"><a
											class="nav-link" id="profile-tab-simple" data-toggle="tab"
											href="#home-simple" role="tab" aria-selected="false">부산</a></li>
										<li class="nav-item myclick" param1="제주"><a
											class="nav-link" id="contact-tab-simple" data-toggle="tab"
											href="#home-simple" role="tab" aria-selected="false">제주</a></li>
									</ul>
									<div class="tab-content" id="myTabContent5">

										<!-- ################### 서울 #################### -->
										<div class="tab-pane fade show active" id="home-simple"
											role="tabpanel" aria-labelledby="home-tab-simple">
											<div class="schedule_info">
												<select class="custom-select s_info" name="s_info"
													id="s_info">
													<option value="default" selected>날짜 / 시간을 선택하세요.</option>
													<c:forEach items="${s_optList}" var="optVo">
														<option value="${optVo.gameDate}|${optVo.gameRound}">${optVo.gameDate}
															R${optVo.gameRound} ${optVo.gameTime}</option>
													</c:forEach>
												</select> <input type="button" class="btn btn-brand btn-sm"
														...
														<div class="card">
															<div class="card-body">
																<div class="d-inline-block">
																	...
																	<h2 class="mb-0">R${sList_infoVo.gameRound}</h2>
																</div>
															</div>
														</div>
													</div>
														...
														<div class="card">
															<div class="card-body">
																<div class="d-inline-block">
																	...
																	<h2 class="mb-0">${sList_infoVo.hClass}</h2>
																</div>
															</div>
														</div>
													</div>
														...
														<div class="card">
															<div class="card-body">
																<div class="d-inline-block">
																	...
																	<h2 class="mb-0">${sList_infoVo.gameName}</h2>
																</div>
															</div>
														</div>
													</div>
														...
														<div class="card">
															<div class="card-body">
																<div class="d-inline-block">
																	...
																	<h2 class="mb-0">${sList_infoVo.gameDistance}</h2>
																</div>
															</div>
														</div>
													</div>
														...
														<tbody>
															<c:forEach items="${sList}" var="svo" varStatus="status">
																<tr>
																	...
																	<td>
																		<div class="m-r-10">
																			<img src="${svo.hPhoto}" class="rounded" width="45">
																		</div>
																	</td>
																	<td><a href="horse_detail?hName=${svo.hName}">${svo.hName}</a></td>
																	<td>${svo.hSex}</td>
																	<td>${svo.hAge}</td>
																	<td>${svo.hWeight}kg</td>
																	<td><a href="player_detail?pName=${svo.pName}">${svo.pName}</a></td>
																</tr>
															</c:forEach>
										...
										<div class="list-group">
											...
											<c:forEach items="${nList}" var="nvo" varStatus="status">
												<div
													...
													</a> <small><p class="mb-1">${nvo.description}</p></small>
												</div>
											</c:forEach>
						...
						<div class="row">
							<div class="card"
								style="width: 1750px; margin-top: 20px; margin-right: 0px;">
								...
								<div class="card-body">
									...
									<div class="row">
										...
										<div class="column"
											style="margin-left: 30px; margin-right: 35px">
											<b>부산경남 렛츠런</b>
											<div id="busanloc"></div>
										</div>
 ...
 $(function(){
	...
	function searchGame(vplace,vg_date,vg_round) {
		...
		var vurl = "";
		if(vg_date == "" && vg_round == ""){
			vurl = "/dark_horse/main/"+vplace
		}
		else{
			vurl = "/dark_horse/main/"+vplace+"/"+vg_date+"/"+vg_round;
		}
		...
		$.ajax({  
			...
			method: 'get',
			contentType: 'application/x-www-form-urlencoded; charset=UTF-8;',
			//data: "gamePlace="+vplace+"&gameDate="+vg_date+"&gameRound="+vg_round,
			dataType: 'json',
			error: function(){alert("에러")},
			success: function(jsonObj){
				...
				$.map(jsonObj.optList, function(opt, i){  // 반드시 요소, 인덱스 인자로 줘야함
					html += "<option value='"+opt.gameDate + "|"+opt.gameRound+"'>" + opt.gameDate + "R"+opt.gameRound+" "+opt.gameTime + "</option> ";
				});
				html += "	</select> " +
				...
				"				<div class='d-inline-block'>" +
				"					<h5 class='text-muted'>Round</h5>" +
				"					<h2 class='mb-0'>R"+jsonObj.hList[0].gameRound+"</h2>" +
				"				</div>" +
				"			</div>" +
				"		</div>" +
				"	</div>" +
				...
				"				<div class='d-inline-block'>" +
				"					<h5 class='text-muted'>등급</h5>" +
				"					<h2 class='mb-0'>"+jsonObj.hList[0].hClass+"</h2>" +
				"				</div>" +
				"			</div>" +
				"		</div>" +
				"	</div>" +
				...
				"				<div class='d-inline-block'>" +
				"					<h5 class='text-muted'>경주명</h5>" +
				"					<h2 class='mb-0'>"+jsonObj.hList[0].gameName+"</h2>" +
				"				</div>" +
				"			</div>" +
				"		</div>" +
				"	</div>" +
				...
				"				<div class='d-inline-block'>" +
				"					<h5 class='text-muted'>거리</h5>" +
				"					<h2 class='mb-0'>"+jsonObj.hList[0].gameDistance+"</h2>" +
				"					</div>" +
				"				</div>" +
				"			</div>" +
				"		</div>" +
				...
				$.map(jsonObj.hList, function(hvo, i){  	// 반드시 요소, 인덱스 인자로 줘야함
					console.log(i + ", " + hvo.hName);
					html += "<tr> " +
					"	<td>"+hvo.gameHno+"</td> " +
					"<td> " +
					"	<div class='m-r-10'> " +
					"		<img src='"+hvo.hPhoto+"' class='rounded' width='45'> " +
					"	</div> " +
					"</td> " +
					"<td><a href=''>"+hvo.hName +"</a></td> " +
					"<td>"+hvo.hSex+"</td> " +
					"<td>"+hvo.hAge+"</td> " +
					"<td>"+hvo.hWeight+"kg</td> " +
					"<td><a href='player_detail?pName="+hvo.pName+"'>"+hvo.pName+"</a></td> " +
					"</tr> ";
